...
import Push_email from "../forms/push_email";
import Send_mail from "../forms/send_mail";
const token = sessionStorage.getItem("auth_token");

class NotificationEmail extends Component {
  state = {
    selectedRowKeys: [],
    searchText: "",
    searchedColumn: "",
  ...
  getData = async () => {
    ...
    const response = await MakeRequestAsync(request_details)
      .then((res) => {
        this.setState({
          load: false,
          data: decryptSingleData(res.data.mail)
        });
      })
      .catch((err) => {
  ...
  render() {
    const columns = [
      ...
      },
      {
        ...
        dataIndex: "html",
        // width: "20%",
        ...this.getColumnSearchProps("html")
      },
      {
        title: "Actions",
        fixed: "right",
    ...
    return (
      <>
        <PageHeader
          ...
          title="Notifications"
          tags={<Tag color="blue">Liste des mails</Tag>}
          subTitle=""
          extra={[<Push_email />]}
        >
          <Row>
