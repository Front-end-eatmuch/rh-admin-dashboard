...
import Support_edit from "../forms/support_edit";
import { decryptSingleData } from "../functions/cryptojs";

const token = sessionStorage.getItem("auth_token");

class SupportSupport extends Component {
  state = {
    selectedRowKeys: [],
    searchText: "",
    searchedColumn: "",
  ...
  getData = async () => {
    ...
    const response = await MakeRequestAsync(request_details)
      .then((res) => {
        this.setState({
          load: false,
          data: decryptSingleData(res.data.support)
        });
      })
      .catch((err) => {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text) => {
          return (
            <Tag
              color={text === 0 ? "orange" : text === 1 ? "green" : "red"}
              key={text}
            >
              {text === 0 ? "Attente" : text === 1 ? "Terminer" : "Rejeter"}
            </Tag>
          );
        }
      },
