...
import Province_edit from "../forms/province_edit";
import Province_new from "../forms/province_new";
...
class Province extends Component {
  ...
  getData = async () => {
    ...
    console.log(decryptSingleData(response.data.province));

    this.setState({
      load: false,
      data: decryptSingleData(response.data.province)
    });
  };
  ...
  handleStatus = async (id, status) => {
    ...
    const request_details = {
      type: UPDATE,
      url: service_api,
      route: province + "/" + update_province + "/" + id,
      data: data,
      token: token
    };
  ...
  handleDelete = async (id) => {
    const request_details = {
      type: DELETE,
      url: service_api,
      route: province + "/" + delete_province + "/" + id,
      token: token
    };
    const response = await MakeRequestAsync(request_details).catch((err) => {
      return openNotificationWithIcon(
    ...
    return (
      <>
        <PageHeader
          ...
          title="Province"
          tags={<Tag color="blue">Liste des provinces</Tag>}
          subTitle=""
          extra={[<Province_new />]}
        >
          <Row>
...
export default connect(mapStateToProps, mapDispatchStoreToProps)(Province);
