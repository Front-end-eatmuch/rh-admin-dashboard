...
import Template_edit from "../forms/template_edit";
import Template_new from "../forms/template_new";
...
class Template extends Component {
  ...
  getData = async () => {
    const request_details = {
      type: GET,
      url: service_api,
      route: template + "/" + get_all_template,
      data: null,
      token: token
    };
    ...
    this.setState({
      load: false,
      data: decryptSingleData(response.data.template)
    });
  };
  ...
  handleStatus = async (id, status) => {
    ...
    const request_details = {
      type: UPDATE,
      url: service_api,
      route: template + "/" + update_template + "/" + id,
      data: data,
      token: token
    };
  ...
  handleDelete = async (id) => {
    const request_details = {
      type: DELETE,
      url: service_api,
      route: template + "/" + delete_template + "/" + id,
      token: token
    };
    const response = await MakeRequestAsync(request_details).catch((err) => {
      return openNotificationWithIcon(
    ...
    return (
      <>
        <PageHeader
          ...
          title="Templates"
          tags={<Tag color="blue">Liste des templates</Tag>}
          subTitle=""
          extra={[<Template_new />]}
        >
          <Row>
...
export default connect(mapStateToProps, mapDispatchStoreToProps)(Template);
